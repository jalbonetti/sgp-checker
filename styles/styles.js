...
export function injectStyles() {
    ...
    style.textContent = `
        ...
        .stc-row-number { font-size: 11px; font-weight: 700; color: var(--stc-text-muted); min-width: 20px; text-align: center; flex-shrink: 0; }
        ...
        .stc-btn-remove { width: 28px; height: 28px; border: 1px solid transparent; border-radius: 6px; background: transparent; color: var(--stc-text-muted); font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all var(--stc-transition); flex-shrink: 0; padding: 0; line-height: 1; }
        .stc-btn-remove:hover { background: var(--stc-danger-bg); border-color: rgba(248,113,113,0.3); color: var(--stc-danger); }
        ...
        .stc-btn-secondary:hover { border-color: var(--stc-text-muted); color: var(--stc-text-primary); }
        .stc-btn-add { background: transparent; border: 1px dashed var(--stc-border); color: var(--stc-text-muted); width: 100%; padding: 10px; }
        .stc-btn-add:hover { border-color: var(--stc-accent); color: var(--stc-accent); background: var(--stc-accent-light); }

        .stc-loading { text-align: center; padding: 32px; color: var(--stc-text-muted); }
        .stc-spinner { display: inline-block; width: 24px; height: 24px; border: 3px solid var(--stc-border); border-top-color: var(--stc-accent); border-radius: 50%; animation: stc-spin 0.8s linear infinite; }
        @keyframes stc-spin { to { transform: rotate(360deg); } }

        .stc-results { margin-top: 20px; }
        ...
        .stc-individual-row { display: flex; align-items: center; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid var(--stc-border); font-size: 13px; gap: 12px; }
        .stc-individual-row:last-child { border-bottom: none; }
        .stc-individual-label { color: var(--stc-text-secondary); flex: 1; }
        .stc-individual-label strong { color: var(--stc-text-primary); }
        .stc-individual-values { display: flex; gap: 16px; flex-shrink: 0; text-align: right; }
        .stc-individual-season, .stc-individual-last30 { min-width: 100px; }
        .stc-individual-season .stc-rate, .stc-individual-last30 .stc-rate { font-weight: 700; color: var(--stc-text-primary); }
        .stc-individual-season .stc-detail, .stc-individual-last30 .stc-detail { font-size: 11px; color: var(--stc-text-muted); }

        .stc-dates-toggle { font-size: 12px; color: var(--stc-accent); cursor: pointer; margin-top: 12px; display: inline-flex; align-items: center; gap: 4px; }
        .stc-dates-toggle:hover { text-decoration: underline; }
        .stc-dates-list { display: none; margin-top: 8px; padding: 10px; background: var(--stc-bg-input); border-radius: 6px; font-size: 12px; color: var(--stc-text-secondary); columns: 3; column-gap: 16px; max-height: 200px; overflow-y: auto; }
        .stc-dates-list.open { display: block; }
        .stc-dates-list span { display: block; padding: 2px 0; }
